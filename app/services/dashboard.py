...
from app.state_vocab import (
    ...
    normalize_load_status_for_reporting,
)
...
def build_dashboard_data(db: Session, limit: int = 25, offset: int = 0) -> DashboardDataResponse:
    ...
    load_status_counts = group_counts(
        db.query(Load.status, func.count(Load.id)).group_by(Load.status).all(),
        normalizer=normalize_load_status_for_reporting,
    )
    ...
    recent_calls = [
        DashboardRecentCall(
            ...
            outcome=normalize_call_outcome_for_reporting(call_session.outcome),
            outcome_tone=outcome_tone(normalize_call_outcome_for_reporting(call_session.outcome)),
            sentiment=normalize_call_sentiment_for_reporting(call_session.sentiment),
            sentiment_tone=sentiment_tone(normalize_call_sentiment_for_reporting(call_session.sentiment)),
            ...
            selected_load=(
                DashboardLoadSnapshot(
                    ...
                    status=normalize_load_status_for_reporting(load.status),
                )
                if load is not None
                else None
            ),
