...
def complete_call(db: Session, payload: CallCompleteRequest) -> CallCompleteResponse:
    ...
    call_session.outcome = payload.outcome
    call_session.sentiment = payload.sentiment
    ...
    if payload.load_id:
        ...
        if payload.final_rate is not None and payload.outcome.lower() in {"agreed", "agreement_reached", "transfer_ready"}:
            load.status = "pending_transfer"
            load_status = load.status

    if payload.final_rate is not None:
